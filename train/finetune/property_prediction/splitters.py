...
tokenizer = T5Tokenizer.from_pretrained("../../../ckpts/molt5-large")
# splitter function
...
def scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    if not return_smiles:
        ...
        train_smiles = [tokenizer(smiles_list[i][1]) for i in train_idx]
        valid_smiles = [tokenizer(smiles_list[i][1]) for i in valid_idx]
        test_smiles = [tokenizer(smiles_list[i][1])for i in test_idx]
        train_data = []
        valid_data = []
        test_data = []
        for i in range(len(train_smiles)):
            data = {
                'graph2d' : train_dataset[i],
                'SMILES': train_smiles[i]
            }
            train_data.append(data)
        for i in range(len(valid_smiles)):
            data = {
                'graph2d' : valid_dataset[i],
                'SMILES': valid_smiles[i]
            }
            valid_data.append(data)
        for i in range(len(test_smiles)):
            data = {
                'graph2d' : test_dataset[i],
