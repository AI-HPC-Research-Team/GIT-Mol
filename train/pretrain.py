...
from models.model_pretrain import GITModel
...
def add_arguments(parser):
    ...
    parser.add_argument("--output_path", type=str, default="ckpts/finetune_ckpts/git_base")
    parser.add_argument("--result_save_path", type=str, default="assets/git_base/result.csv")
...
if __name__ == "__main__":
    ...
    train_data_file = "../../igcdata/train_4m.pkl"
    val_data_file = "../../igcdata/valid_400k.pkl"

    #train_data_file = "../../igdata/train_220k.pkl"
    #val_data_file = "../../igdata/valid_20k.pkl"
    ...
    if args.mode == "train":
        ...
        model = GITModel(config = config['network'], modal = task, device = accelerator.device)
        ...
        logger.info(f"mode : {args.mode} ")
        test_dataset = gitDataset(train_data_file, config["data"]["drug"], split="val", task = task)
        for i in range(2):
        ...
        model = GITModel(config = config['network'], modal = task, device = args.device)
