...
def text_evaluate(tk, targets, preds, molecules, text_trunc_length):
    ...
    result_dataframe = pd.DataFrame(outputs, columns=['smi', 'gt', 'out'])

    for i, (smi, gt, out) in enumerate(outputs):

        rs = scorer.score(out, gt)
        rouge_scores.append(rs)
    meteor_scores_df = pd.DataFrame(meteor_scores)
    # print('ROUGE score:')
    ...
    result_dataframe = pd.concat([result_dataframe, meteor_scores_df], axis=1)

    return bleu2, bleu4, rouge_1, rouge_2, rouge_l, _meteor_score, result_dataframe
